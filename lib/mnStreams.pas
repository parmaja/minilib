...
begin
  ...
  try
    repeat
      n := Source.Read(aBuffer^, ReadWriteBufferSize);
      if n > 0 then
        Write(aBuffer^, n);
      Inc(Result, n);
    until (n < ReadWriteBufferSize) or not Connected;
  finally
    FreeMem(aBuffer, ReadWriteBufferSize);
  end;
end;
...
begin
  inherited Create;
  FZeroClose := True;
  FReadBufferSize := ReadWriteBufferSize;
