...
uses
  SysUtils, Variants, Classes, Contnrs, intffields;
...
type
  ...
  IParams = interface(IStreamPersist) ['{4F35798E-8B1B-4FC2-9710-AAC058122A6D}']
    ...
    procedure SetCarried(Value:TObject);
    function GetCarried:TObject;
    ...
    procedure Clear;
    property Carried: TObject read GetCarried write SetCarried;
...
function TIParams.GetCarried: TObject;
begin
  Result := FCarried;
end;

procedure TIParams.SetCarried(Value: TObject);
begin
  if FCarried <> nil then
    FCarried.Free;
  FCarried := Value;
end;
