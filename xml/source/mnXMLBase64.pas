...
{******************************************************************************}
{
  Ported to minilib to use it in Delphi
}
...
begin
  ...
  begin
    ...
    OutBuf[0] := Base64Table[((InBuf[0] and $FC) shr 2) + 1];
    OutBuf[1] := Base64Table[(((InBuf[0] and $03) shl 4) or ((InBuf[1] and $F0) shr 4)) + 1];
    OutBuf[2] := Base64Table[(((InBuf[1] and $0F) shl 2) or ((InBuf[2] and $C0) shr 6)) + 1];
    OutBuf[3] := Base64Table[(InBuf[2] and $3F) + 1];
...
var
  aIn: TBase64EncodingStream;
  aOut: TStringStream;
begin
  aOut := TStringStream.Create('');
  try
    aIn :=  TBase64EncodingStream.Create(aOut);
    try
      aIn.Write(PChar(S)^, Length(S));
    finally
      aIn.Free;
    end;
    Result := aOut.DataString;
  finally
    aOut.Free;
  end;
end;
...
var
  aIn: TStringStream;
  aOut: TBase64DecodingStream;
  aBuffer:array[0..255] of Char;
  c:Integer;
begin
  Result := '';
  aIn := TStringStream.Create(S);
  try
    aOut := TBase64DecodingStream.Create(aIn);
    try
      while not aOut.EOF do
      begin
        c := aOut.Read(aBuffer, SizeOf(aBuffer));
        Result := Result + Copy(aBuffer, 1, c);
      end;
    finally
      aOut.Free;
    end;
