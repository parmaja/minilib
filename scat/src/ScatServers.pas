...
type
  TScatConnection = class;
  TScatConnectionClass = class of TScatConnection;
  TScatGetFlag = (hgSendDoc, hgSendStream, hgWillSendMySelf, hg404, hgAcceptData);
  TScatSendType = (ScatSendHead, ScatSendDoc);
  TScatConnectionState = (hcRequest, hcHeader, hcPostedData);

  TScatConnection = class(TmnServerConnection)
  private
  ...
  TmnScatListener = class(TmnListener)
  private
    FDocumentRoot: string;
    FAddress: string;
    FPort: Integer;
  ...
  TOnScatServerEvent = procedure(Sender: TObject; Socket: TScatConnection) of object;

  TmnScatServer = class(TmnServer)
  private
    FDocumentRoot: string;
    FDefaultDocument: TStringList;
...
constructor TmnScatServer.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TmnScatServer.Destroy;
begin
  FreeAndNil(FDefaultDocument);
  inherited;
end;

procedure TmnScatServer.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
end;

constructor TScatConnection.Create(Socket: TmnCustomSocket);
begin
  inherited;
...
destructor TScatConnection.Destroy;
begin
...
function TScatConnection.GetDocument(Document: string): string;
begin
  Listener.Enter;
  try
    Result := (Listener as TmnScatListener).GetDocument(Document);
  finally
    Listener.Leave;
  end;
end;
...
procedure TScatConnection.ReceiveData;
...
procedure TScatConnection.Answer404;
var
  Body: string;
begin
...
procedure TScatConnection.ProcessRequest;
begin
...
procedure TScatConnection.ProcessPost;
begin
  Answer404;
end;

procedure TScatConnection.ProcessHead;
begin
  AnswerDocument;
end;

procedure TScatConnection.ProcessGet;
begin
  AnswerDocument;
end;
...
procedure TScatConnection.AnswerDocument;
var
  DocSize: Int64;
...
procedure TScatConnection.Process;
begin
...
procedure TmnScatServer.SetDefaultDoc(const Value: TStringList);
begin
  FDefaultDocument.Assign(Value);
end;

function TmnScatServer.CreateListener: TmnListener;
begin
...
function TmnScatListener.GetDocument(Root: string): string;
var
  i: Integer;
begin
...
function TmnScatListener.CreateConnection(vSocket: TmnCustomSocket): TmnServerConnection;
begin
  Result := TScatConnection.Create(vSocket);
  (Result as TScatConnection).DocumentRoot := FDocumentRoot;
end;

procedure TmnScatListener.SetDefaultDocument(const Value: TStringList);
begin
  FDefaultDocument := Value;
end;

constructor TmnScatListener.Create;
begin
  inherited;
...
destructor TmnScatListener.Destroy;
begin
