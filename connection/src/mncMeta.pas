...
{ TmncMeta }
  ...
  TmncMetaClassItem = class(TObject)
  public
    Name: string;
    Title: string;
  ...
  TmncMetaClasses = class(TObjectList)
  private
    function GetItems(Index: Integer): TmncMetaClassItem;
  public
    function RegisterMeta(vName, vTitle: string; vMetaClass: TmncMetaClass; vConnectionClass: TmncConnectionClass): TmncMetaClassItem;
    function Find(vName: string): TmncMetaClassItem; overload;
    function Find(vConnectionClass: TmncConnectionClass): TmncMetaClassItem; overload;
    function IndexOf(vName: string): Integer;
    function CreateMeta(vModel: string): TmncMeta; overload;
    function CreateMeta(vConnection: TmncConnection): TmncMeta; overload;
    procedure EnumConnectionsModels(Strings: TStrings);
    property Items[Index:Integer]: TmncMetaClassItem read GetItems; default;
  end;

function Meta: TmncMetaClasses;

implementation

var
  FMeta: TmncMetaClasses;

function Meta: TmncMetaClasses;
begin
  if FMeta = nil then
    FMeta := TmncMetaClasses.Create;
  Result := FMeta;
end;

{ TmncMetaClasses }

function TmncMetaClasses.GetItems(Index: Integer): TmncMetaClassItem;
begin

end;

function TmncMetaClasses.RegisterMeta(vName, vTitle: string; vMetaClass: TmncMetaClass; vConnectionClass: TmncConnectionClass): TmncMetaClassItem;
begin
  Result := Find(vName);
  if Result <> nil then
    raise exception.Create(vName + ' is already registered');
  Result := TmncMetaClassItem.Create;
  Result.Name := vName;
  Result.Title := vTitle;
...
function TmncMetaClasses.Find(vName: string): TmncMetaClassItem;
var
  i: Integer;
begin
  Result := nil;
...
function TmncMetaClasses.Find(vConnectionClass: TmncConnectionClass): TmncMetaClassItem;
var
  i: Integer;
begin
  Result := nil;
  for i := 0 to Count -1 do
  begin
    if vConnectionClass.InheritsFrom(Items[i].ConnectionClass) then
    begin
      Result := Items[i];
      break;
    end;
...
function TmncMetaClasses.IndexOf(vName: string): Integer;
var
  i: Integer;
begin
  Result := -1;
...
function TmncMetaClasses.CreateMeta(vModel: string): TmncMeta;
var
  P: TmncMetaClassItem;
begin
  Result := nil;
  ...
  if P <> nil then
    ...
    raise EmncException.Create('Mata ' + vModel + ' not found');
end;

function TmncMetaClasses.CreateMeta(vConnection: TmncConnection): TmncMeta;
var
  P: TmncMetaClassItem;
begin
  Result := nil;
  P := Find(TmncConnectionClass(vConnection.ClassType)); //TODO
  if P <> nil then
    Result := P.MataClass.Create
  else
    raise EmncException.Create('Mata ' + vConnection.ClassName + ' not found');
end;

procedure TmncMetaClasses.EnumConnectionsModels(Strings: TStrings);
var
  item: TmncMetaClassItem;
begin
  for item in Self do
  begin
